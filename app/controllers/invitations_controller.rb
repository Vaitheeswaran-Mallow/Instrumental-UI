class InvitationsController < ApplicationController
  ...
  before_action :require_admin_role, except: [:accept_invitation, :process_invitation]
  ...
  def new
    ...
    @invitation.set_default_message
  end
  ...
  def create
    ...
    if @invitation.save
      ...
      redirect_to invitations_path(@account.slug), notice: "Invitation sent successfully!"
    else
      render :new
    end
  end
  ...
  def update
    ...
    if @invitation.update(invitation_params)
      redirect_to invitations_path(@account.slug), notice: "Invitation updated successfully!"
    else
      render :edit
    end
  end
  ...
  def process_invitation
    ...
    if User.exists?(email_address: params[:email_address])
      redirect_to new_session_path, alert: "A user by this email already exists."
      return
    end
    ...
    if @user.save
      ...
      redirect_to accounts_path, notice: "Welcome!"
  ...
  def destroy
    ...
    redirect_to invitations_path(@account.slug), notice: "Invitation deleted successfully!"
  end
